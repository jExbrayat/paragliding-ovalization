class SetOfConditions {
    ...
    private wing_vz: number;
    private wing_vx: number;
    private pilot_upwind_vx: number;
    private pilot_vz: number;

    constructor(wind_vx: number, thermal_vz: number, wing_vz: number, wing_vx: number) {
...
document.addEventListener('DOMContentLoaded', function () {
    function compute_solution(wind_vx: number, thermal_vz: number, time_for_one_turn: number, wing_vz: number, wing_vx: number): number {
    ...
    function updatePlot(wind_vx: number, time_for_one_turn: number, wing_vz: number, wing_vx: number): void {
        ...
        const x_axis: number[] = Array.from({length: Math.ceil((10 - 1.5) / 0.5) + 1}, (_, i) => 1.5 + (i * 0.5));
    ...
    // Make initial plot
    const sliderObject = document.getElementById("windSlider") as HTMLInputElement;
    const inputObject1 = document.getElementById("input1") as HTMLInputElement;
    const inputObject2 = document.getElementById("input2") as HTMLInputElement;
    const inputObject3 = document.getElementById("input3") as HTMLInputElement;
